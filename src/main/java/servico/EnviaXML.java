				...
				if (arq.exists()) { // se o arquivo foi gerado normalmente
					...
					retornoEnvio = aut.enviaXml(Config.token, Config.x_integration_key, nomeArquivo); // envia o arquivo
					 ...
					 if (retornoEnvio.getStatus().getCode().equals("E2")||
						...
						ret = aut.confirmaProcessamento(Config.token, Config.x_integration_key, retornoEnvio.getId());// envia a chave pra confirmação
						...
						while (ret.getFilesExpanded().getApiStatus().getCode().equals("SA1")) { // enquanto esse xml não for processado ele continua enviando
							System.out.println("Nova tentativa de confirmação de processamento...");
							ret = aut.confirmaProcessamento(Config.token, Config.x_integration_key, retornoEnvio.getId()); 
							System.out.println(status);
						}
						if (ret.getFilesExpanded().getApiStatus().getCode().equals("SA2")||ret.getFilesExpanded().getApiStatus().getCode().equals("EA10")) { // se estiver tudo certo dá update pra cofre = 1 e deleta o arquivo
							GregorianCalendar g = new GregorianCalendar();
